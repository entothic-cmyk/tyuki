...
import { BOTLIY_API_KEY, BOTLIY_BASE_URL } from './_config.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const upstream = await fetch(`${BOTLIY_BASE_URL}/chat/completions`, {
      method: 'POST',
      headers: {
    ...
    if (chatId && fullReply) {
      ...
      if (chats.length) {
        ...
        await sql`
          ...
          VALUES (${chatId}, 'user', ${lastUser?.content || ''}, ${JSON.stringify(lastUser?.attachments || [])}, ${Date.now()})
        `;
        await sql`
          INSERT INTO messages (chat_id, role, content, attachments, created_at)
